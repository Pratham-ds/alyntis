...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { label: 'Quizzes', value: stats.quizzes, icon: HelpCircle, to: '/admin/projects', color: 'from-navy-700 to-navy-900' },
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            <p className="mt-1 text-sm text-gray-400">
              Welcome back, {profile?.full_name || profile?.email}. Manage your platform here.
            </p>
          </div>
            ...
            <>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {statCards.map((stat) => (
                  <Link
                    key={stat.label}
                    to={stat.to}
                    className="group rounded-2xl border border-white/10 bg-white/5 p-6 transition-all hover:border-teal-500/30 hover:bg-white/10"
                  >
              ...
              <div className="mt-8">
                <div className="flex items-center justify-between">
                  ...
                  <Link to="/admin/notifications" className="text-sm font-semibold text-teal-400 hover:text-teal-300">
                    View all →
                  </Link>
                </div>
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-white">
                            ...
                            <span className="font-normal text-gray-400">
                              {notif.type === 'project_completion' && `completed ${notif.project_title}`}
                              {notif.type === 'quiz_completion' && `passed ${notif.quiz_title}`}
                              {notif.type === 'quiz_attempt' && `attempted ${notif.quiz_title}`}
                            </span>
                          </p>
                        ...
                        {notif.score !== null && (
                          <span className={`rounded-full px-3 py-1 text-xs font-bold ${
                            ...
                            {notif.score}/{notif.total_questions ?? 0}
                          </span>
                        )}
                      </div>
                    ))
