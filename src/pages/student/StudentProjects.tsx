...
const classFilters = ['All', 'Class 6', 'Class 7', 'Class 8', 'Class 9', 'Class 10', 'Senior'];
...
export default function StudentProjects() {
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter((project) => {
      if (selectedClass !== 'All' && project.class_level !== selectedClass) return false;
      ...
      <div className="min-h-screen bg-navy-950 pt-20">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-gray-400">Browse projects, access resources and take quizzes.</p>
          </div>
          ...
          <div className="mb-6 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="flex items-center gap-1 text-xs font-semibold text-gray-400">
                <SlidersHorizontal className="h-3.5 w-3.5" /> Class:
              </span>
              {classFilters.map((cls) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                    selectedClass === cls ? 'bg-teal-500 text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10'
                  }`}
                >
                  {cls}
                </button>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="flex items-center gap-1 text-xs font-semibold text-gray-400">
                <SlidersHorizontal className="h-3.5 w-3.5" /> Tech:
              </span>
              {techFilters.map((tech) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                    selectedTech === tech ? 'bg-navy-700 text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10'
                  }`}
                >
                  {tech}
                </button>
                  ...
                  >
                    {project.image_url && (
                      <div className="relative h-36 overflow-hidden">
                        ...
                        {isCompleted && (
                          <div className="absolute top-2 right-2 flex items-center gap-1 rounded-full bg-teal-500 px-2 py-1 text-xs font-bold text-white">
