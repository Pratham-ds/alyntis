import { ArrowRight, Bot, BrainCircuit, Code2, Cpu, Plane, Radio } from 'lucide-react';
            ...
            <Reveal key={item.level} delay={index * 60}>
              <div className="relative h-full rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase tracking-wider text-teal-600">{item.level}</span>
                  <item.icon className="h-5 w-5 text-navy-700" />
                </div>
                <h3 className="mt-5 text-base font-bold text-navy-900">{item.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-gray-600">{item.desc}</p>
                {index < levels.length - 1 && <div className="mt-5 hidden h-px w-8 bg-teal-200 md:block" />}
              </div>
            </Reveal>
          ))}
        </div>
